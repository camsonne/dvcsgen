  ...
  TGenDVCS::TGenDVCS(Double_t Ebeam, Int_t TargType, UInt_t seed1, UInt_t seed2):TGenBase(Ebeam,seed1,seed2)
{
 ...
 tOut->Branch("k",&feini,32000,1);
  tOut->Branch("pini",&fpini,32000,1);
  tOut->Branch("p",&fp,32000,1);
  tOut->Branch("kp",&fe,32000,1);
  tOut->Branch("kpv",&feprerad,32000,1);
  tOut->Branch("q",&fq,32000,1);
  tOut->Branch("vertex",&fVertex,32000,1);
  tOut->Branch("Q2",&fQ2);
  tOut->Branch("xb",&fxb);
  tOut->Branch("Q2",&fQ2);
  tOut->Branch("SigmaP",&fSigmaP);
  tOut->Branch("SigmaM",&fSigmaM);
  tOut->Branch("PSF",&fPSF);
  tOut->Branch("t",&ft);
  tOut->Branch("s",&fs);
  tOut->Branch("tmin",&ftmin);
  tOut->Branch("tmax",&ftmax);
  tOut->Print();   
